...
export default function WithdrawPage() {
  ...
  function calculatePreview() {
    ...
    if (selectedMethod === 'crypto') {
      ...
      const ngnAmount = amountNum * exchangeRate;
      fee = 100 / exchangeRate; // ₦100 fee converted to USD
      net = amountNum - fee;
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!wallet || amountNum > wallet.available_balance) {
      setError(`Insufficient balance. Available: $${wallet?.available_balance}`);
      setSubmitting(false);
      return;
    }
    ...
    return (
      <div className="py-8 max-w-lg mx-auto text-center">
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          <h2 className="text-xl font-semibold text-yellow-800 mb-2">⚠️ No Withdrawal Options Set</h2>
  ...
  return (
    <div className="py-8 max-w-lg mx-auto">
      ...
      <p className="text-gray-600 mb-6">
        ...
        <span className="text-sm text-gray-400 ml-2">(≈ ₦{((wallet?.available_balance || 0) * exchangeRate).toLocaleString()})</span>
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            type="number"
            step={selectedMethod === 'crypto' ? '1' : '0.01'}
            min={minAmount}
            max={maxAmount}
            value={amount}
            onChange={(e) => setAmount(e.target.value)}
            placeholder={`Min $${minAmount.toFixed(2)} - Max $${maxAmount.toFixed(2)}`}
